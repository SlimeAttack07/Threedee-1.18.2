...
public class HeadAssemblerCategory implements IRecipeCategory<HeadAssemblerRecipe>{
	private final IDrawable icon;
	private final IDrawable background;

	public HeadAssemblerCategory(IGuiHelper guihelper) {
		icon = guihelper.createDrawableIngredient(VanillaTypes.ITEM, new ItemStack(TDBlocks.HEAD_ASSEMBLER.get()));
		background = guihelper.createDrawable(Constants.RECIPE_GUI_VANILLA, 0, 168, 125, 18);
	}
	
	@Override
	public ResourceLocation getUid() {
		return new ResourceLocation(Threedee.MOD_ID, "head_assembler");
	}

	@Override
	public Class<? extends HeadAssemblerRecipe> getRecipeClass() {
		return HeadAssemblerRecipe.class;
	}

	@Override
	public Component getTitle() {
		return new TranslatableComponent("jei.threedee.titles.head_assembler");
	}
	...
	@Override
	public void setRecipe(IRecipeLayoutBuilder builder, HeadAssemblerRecipe recipe, IFocusGroup focuses) {
	...
	@Override
	public void draw(HeadAssemblerRecipe recipe, IRecipeSlotsView view, PoseStack stack, double mouseX, double mouseY) {
		String text;
